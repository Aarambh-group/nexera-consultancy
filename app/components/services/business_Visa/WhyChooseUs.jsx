"use client";

import { motion } from "framer-motion";
import { Building2, FileCheck, Clock, ShieldCheck } from "lucide-react";

const luxuryEase = [0.16, 1, 0.3, 1];

const benefitsList = [
  {
    icon: Building2,
    title: "Corporate Desk Specialists",
    desc: "Tailored immigration handling specifically for business executives, delegates, and entrepreneurs.",
  },
  {
    icon: FileCheck,
    title: "Invitation Accuracy",
    desc: "Rigorous alignment of host invitations and employer dispatch letters to satisfy consulate requirements.",
  },
  {
    icon: Clock,
    title: "Express Processing",
    desc: "Priority appointment scheduling and fast-track submission assistance for urgent meetings.",
  },
  {
    icon: ShieldCheck,
    title: "High Approval Track Record",
    desc: "Detailed documentation audits designed to prevent delays or refusals for time-sensitive travel.",
  },
];

export default function WhyChooseUs() {
  return (
    <section className="py-8 sm:py-12 lg:py-16 bg-white font-sans selection:bg-[#E5A64C] selection:text-[#0A1329]">
      {/* Aligned container matching design system standards */}
      <div className="max-w-7xl mx-auto px-4 sm:px-8 lg:px-12 w-full space-y-8 sm:space-y-10">
        
        {/* Header Block */}
        <div className="max-w-xl sm:max-w-2xl mx-auto text-center space-y-3">
          {/* Subtitle Badge */}
          <motion.div
            initial={{ opacity: 0, y: 12 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6, delay: 0.1, ease: luxuryEase }}
            className="flex items-center justify-center gap-2"
          >
            <span className="relative inline-block text-[10px] sm:text-xs tracking-[0.1em] uppercase text-[#E5A64C] font-semibold after:absolute after:left-1/2 after:-translate-x-1/2 after:-bottom-1 after:h-[2px] after:w-8 after:bg-[#E5A64C]">
              WHY CHOOSE US
            </span>
          </motion.div>

          {/* Headline */}
          <motion.h2
            initial={{ opacity: 0, y: 16 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.7, delay: 0.2, ease: luxuryEase }}
            className="text-2xl sm:text-3xl lg:text-4xl font-serif leading-tight text-[#0A1329] tracking-tight"
          >
            Why Rely on Nexera for{" "}
            <span className="italic font-normal text-[#E5A64C]">
              Business Visas
            </span>
          </motion.h2>
        </div>

        {/* Benefits Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {benefitsList.map((item, idx) => {
            const Icon = item.icon;
            return (
              <motion.div
                key={idx}
                initial={{ opacity: 0, y: 15 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.5, delay: idx * 0.08, ease: luxuryEase }}
                className="group bg-slate-50/70 border border-slate-200/80 rounded-xl p-5 space-y-3 shadow-sm hover:border-[#E5A64C]/50 hover:bg-white hover:shadow-md transition-all duration-300"
              >
                <div className="w-10 h-10 rounded-lg bg-[#070D1F] flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform duration-300">
                  <Icon className="w-5 h-5 text-[#E5A64C]" />
                </div>
                <div className="space-y-1.5">
                  <h3 className="text-sm font-semibold text-[#0A1329]">
                    {item.title}
                  </h3>
                  <p className="text-xs text-slate-500 font-light leading-relaxed">
                    {item.desc}
                  </p>
                </div>
              </motion.div>
            );
          })}
        </div>

      </div>
    </section>
  );
}